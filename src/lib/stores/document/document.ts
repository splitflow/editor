...
// public interface
export interface Document extends Readable<BlockNode[]> {
    get(): BlockNode[]
}
	...
	const { subscribe } = writable<BlockNode[]>(value, (set) => {
		const unsubscribe1 = fragments.subscribe(($fragments) => {
            ...
            if (newFragments.length === 0) {
                ...
                // we have to recompute all fragments
                newFragments = $fragments
                fragmentsSnapshot = undefined
            }

            position = $fragments.length
			fragmentsSnapshot = snapshotFragments(fragmentsSnapshot, newFragments)
