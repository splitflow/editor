...
import createDocumentStore, { type Document, type DocumentStore } from './stores/document/document'
...
export class EditorModule {
	...
	async upload(file: File) {
		...
		const result = await this.dispatcher.dispatchAsyncAction<UploadResult>(action, {
			discriminator: this
		})
		return result.url
	}
