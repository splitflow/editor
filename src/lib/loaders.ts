...
import { data, insertPosition, key, type BlockNode, type DocumentNode } from './document'
 ...
 * Positions are reset
 */
export function createDocumentBundle(blocks: BlockNode[]): DocumentBundle {
	const document = {}

	let position: number
	for (const block of blocks) {
		position = insertPosition(position)
		document[key(block)] = { ...data(block), position }
	}
    
	return { options: { documentId: undefined }, getDocumentResult: { document } }
}
